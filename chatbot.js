...
(function () {
  ...
  let isVoiceInput = false; // Variável para saber se usaste o microfone
  ...
  .msg.bot .msg-bubble{ background:rgba(0,43,73,.6); color:#f0f4f8; border-bottom-left-radius:4px; }
  ...
  micBtn.addEventListener('click', () => {
    isVoiceInput = true; // Ativa o modo de voz
    recognition.start();
  ...
  async function sendMsg(text) {
    ...
    try {
      ...
      if (data.reply) {
        ...
        // SÓ FALA SE O INPUT VEIO DO MICROFONE
        if (isVoiceInput) {
          speak(data.reply);
          isVoiceInput = false; // Reset para a próxima
        }
      }
    } catch (e) {
  ...
  document.getElementById('chatSend').onclick = () => {
    isVoiceInput = false; // Garante que se clicar no botão de enviar, não fala
  ...
  document.getElementById('chatInput').onkeydown = (e) => {
    if (e.key === 'Enter' && !e.shiftKey) {
      e.preventDefault();
      isVoiceInput = false; // Se usar o teclado, não fala
